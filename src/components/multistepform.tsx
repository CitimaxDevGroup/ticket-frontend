...
export default function MultiStepForm() {
  ...
  try {
    ...
    const response = await fetch("https://script.google.com/macros/s/AKfycbzoZrQdkGyEemKvhcSTESk0HNiHEdZ4t4zx0S2nW9MgmFwmEMFg4Ly9gFr8SSLxLHOcUQ/exec", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      // ⚠️ Only use "no-cors" if you cannot modify the Apps Script to return CORS headers
      mode: "no-cors",
      body: JSON.stringify(payload),
    });

    try {
      const text = await response.text(); // will fail if using "no-cors"
      ...
      try {
        ...
        alert("Unexpected server response (not valid JSON).");
        return;
      }
